...
void links_reserve(t_link_array *A, int min_cap)
{
    if (A->phys_size >= min_cap) return;

    int new_cap;
    if (A->phys_size == 0){new_cap = 8;}
    else {new_cap = A->phys_size * 2;}
...
int links_contains( t_link_array *A, int from, int to)
{
...
void links_add_unique(t_link_array *A, int from, int to)
{
    if (from == to) return;
...
t_link_array hasse_build_links( t_partition *part,  adjlist_t *graph)
{
    ...
    for (int i = 0; i < n; ++i) {
        int Ci = v2c[i];
        // Remplacement de la boucle for complexe par un while plus classique
        cell_t *element = graph->lists[i].head;
        
        while (element != NULL)
        {
            int j0 = element->dest - 1;
            int Cd = v2c[j0];
            if (Ci != Cd){links_add_unique(&A, Ci, Cd);}
